...
export { normalizeJid, safeGroupOperation, extractUserFromJid };
...
export function makeWASocket(connectionOptions, options = {}) {
  ...
  const sendBackoffUntil = new Map()
  const SEND_GAP_MS = Number(process.env.WA_SEND_GAP_MS || 1200)
  const RATE_LIMIT_RETRIES = Number(process.env.WA_RATE_LIMIT_RETRIES || 3)
  const RATE_LIMIT_RETRY_MS = Number(process.env.WA_RATE_LIMIT_RETRY_MS || 8000)

  const delay = (ms) => new Promise(resolve => {
    setTimeout(resolve, ms)
  })
  const waitForSendReady = async () => {
    if (global.whatsappSendReady !== false) return
    const startedAt = Date.now()
    while (global.whatsappSendReady === false && Date.now() - startedAt < 15000) {
      await delay(500)
    }
  }
  ...
  const runQueuedSend = async (jid, task) => {
    ...
    try {
      ...
      setTimeout(() => {
        release()
        if (sendQueues.get(jid) === queued) sendQueues.delete(jid)
      }, SEND_GAP_MS)
    }
  }
  ...
  conn.sendMessage = async function(jid, message, options = {}) {
    const sendOnce = async () => {
      if (!jid) {
        throw new Error('Cannot send message: missing chat JID');
      }

      if (!process.env.ELTA_CHILD_PROCESS && process.env.DEBUG_SEND_MESSAGE === 'true') {
        console.log(`[DEBUG] Queueing WhatsApp message to ${jid}`);
      }
      ...
      if (!process.env.ELTA_CHILD_PROCESS && process.env.DEBUG_SEND_MESSAGE === 'true') {
        if (result?.key?.id) {
          console.log(`[DEBUG] WhatsApp accepted message for ${jid}, ID: ${result.key.id}`);
        } else {
          console.warn(`[DEBUG] WhatsApp send returned no message key for ${jid}`);
        }
      }
    ...
    try {
      return await runQueuedSend(jid || 'unknown', async () => {
        await waitForSendReady()
        const backoffUntil = sendBackoffUntil.get(jid) || 0
        ...
        for (let attempt = 0; attempt <= RATE_LIMIT_RETRIES; attempt++) {
          try {
            ...
            if (!isRateLimitError(error) || attempt === RATE_LIMIT_RETRIES) throw error

            const retryDelay = RATE_LIMIT_RETRY_MS * (attempt + 1)
            sendBackoffUntil.set(jid, Date.now() + retryDelay)
            if (!process.env.ELTA_CHILD_PROCESS) {
              console.warn(`[SEND RETRY] ${jid}: rate limited, retrying in ${retryDelay}ms`)
            }
      ...
      if (!process.env.ELTA_CHILD_PROCESS) {
        console.error(`[SEND ERROR] ${jid || 'unknown chat'}: ${error.message}`);
      }
      throw error
    }
  }
...
reply: {
       ...
       async value(jid, text = '', quoted, options) {
         const chatSettings = global.db?.data?.chats?.[jid] || {};
         const userJid = quoted?.sender || jid;

         if (chatSettings.noisy_to_dm && jid.endsWith('@g.us')) {
             // Send to user's DM instead of group
             return conn.sendMessage(userJid, { text: text }, { ...options });
         }
